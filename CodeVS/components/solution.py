...
class Solution:
    ...
    def __iterate_assign_barges(self, order, barges, days, limit_assign_capacity, is_assign_capacity = False, used_barges_ids = set()):
        # Filter available barges that are free during order time window and ready
        assigned_barges = []
        remaining_demand = order.demand
        if is_assign_capacity:
            remaining_demand = limit_assign_capacity
        ...
        for barge in sorted_barges:
            ...
            if barge.barge_id in used_barges_ids:
                continue
            ...
            total_load += assign_amount
            if total_load >= limit_assign_capacity:
                break
    ...
    def assign_barges_to_single_order(self, order, barges, limit_assign_capacity = 1000000, 
        ...
        #print("DEBUG1:", total_capacity, order.demand, limit_assign_capacity)
        
        iteration = 0
        while total_capacity < order.demand and total_capacity < limit_assign_capacity and iteration < 10:
            assigned_barges = self.__iterate_assign_barges(order, barges, days, limit_assign_capacity, is_assign_capacity, used_barges_ids)
            days = days + 2
            total_capacity = sum(barge['barge'].capacity for barge in assigned_barges)
            iteration += 1
    ...
    def assign_barges_to_tugboats(self, order, tugboats, order_assigned_barges):
        return self.assign_barges_to_tugboats_non_order(order.order_id, order.start_object.station, order.start_datetime,
                                                        order.due_datetime, tugboats, order_assigned_barges)
        
        #raise Exception("No tugboat found for order 30 days: " + str(order.order_id))
    
    def assign_barges_to_tugboats_non_order(self, order_id, start_station, start_datetime, due_datetime, 
                                            tugboats, order_assigned_barges, travel_before_days=0):
        assigned_tugboats = []
        
        sorted_tugboats = self.code_info.get_code_next_tugboat(start_station, 
                                                               start_datetime,
                                                               due_datetime, tugboats, 
                                                               config_problem.RELAX_DAYS+travel_before_days)
        ...
        for tugboat in sorted_tugboats:
            tugboat_ready_time = self.get_ready_time_tugboat(tugboat)
            tugboat_id = tugboat.tugboat_id
            if due_datetime < tugboat_ready_time:
                #print("Tugboat is not ready",tugboat_id, order_id, due_datetime, tugboat_ready_time)  
                continue
        ...
        if len(assigned_tugboats) != 0 and len(order_assigned_barges) != len(copy_order_assigned_barges):
            ...
            #raise Exception("No tugboat found for order: " + str(order_id))
        ...
        sorted_tugboats = self.code_info.get_code_next_tugboat(start_station, 
                                                               start_datetime, 
                                                               due_datetime, 
                                                               tugboats, config_problem.MAX_RELAX_DAYS+travel_before_days)
        ...
        if len(assigned_tugboats) != 0:
            return True, assigned_tugboats
        for tugboat in sorted_tugboats:
            print("Tugboat", tugboat.tugboat_id, len(tugboat.assigned_barges))
    ...
    def update_collection_barge_time_none_order(self, start_datetime, collection_time_info):
        if True:
            #print("-------------------------------------------------------------")
            #print("Order Type: ", order.order_type)
            #print("Collection Barge Infos:", collection_time_info)
            #print("-------------------------------------------------------------")
            ORDER_DATETIME = start_datetime
    ...
    def update_collection_barge_time(self, order, tugboat, collection_time_info):
        self.update_collection_barge_time_none_order(order.start_datetime, collection_time_info)
    
    def update_travel_info(self, order, tugboat, travel_time_info):  
        return self.update_travel_info_none_order(order.start_datetime, travel_time_info)
            
    def update_travel_info_none_order(self, start_datetime, travel_time_info):  
    ...
    def arrival_step_transport_order(self, order, assigned_tugboats, order_trip = 1):
        ...
        for tugboat in assigned_tugboats:
            ...
            travel_info = tugboat.calculate_travel_first_barge( self.barge_scheule)
            
            
            #raise Exception("Travel info")``
        ...
        # print(total_time, travel_info)
            ...
            isDeBug = False
            barge_ids = []
            barge_location.exit_datetime = barge_location.enter_datetime
    ...
    def arrival_step_river_transport(self, order, river_tugboats, lookup_barge_infos, 
                                     sea_tugboat_results, round_order_trip):
        """
    ...
    def arrival_step_travel_empty_barges(self, order, tugboats, appointment_infos, 
                                         order_trip, is_startdate_order=False):
        ...
        for tugboat in tugboats:
            tugboat_info = self.tugboat_scheule[tugboat.tugboat_id][-1]
            tugboat_ready_time = self.get_ready_time_tugboat(tugboat)
            #print("TUGBOAT READY TIME:", tugboat_ready_time)
            collection_time_info = tugboat.calculate_collection_barge_time(tugboat_info, self.barge_scheule)
            if is_startdate_order:
                #start_datetime start before 5 days order.start_datetime
                start_datetime = order.start_datetime - timedelta(days=5)
                if tugboat_ready_time > start_datetime:
                    start_datetime = tugboat_ready_time
                    # 
            ...
            first_barge_location = collection_time_info['barge_collect_infos'][0]
           # print("FIRST BARGE LOCATION:", first_barge_location)
            tugboat_ready_time = self.get_ready_time_tugboat(tugboat)
            
            start_location = DataPoint(
                ID="Start",
                type="Start",
                name=f"Start Collect Barge River Down From {start_station.station_id} To {end_station.station_id}",
                enter_datetime=tugboat_ready_time,
                #exit_datetime=tugboat_ready_time,
                distance=0,
                time=0,
            ...
            #print("BARGE READY TIME:", barge_ready_time)
            barge_location = DataPoint(
                ID="Barge",
                type="Barge Collection",
                name="Barge Location",
            ...
            for collection_info in collection_time_info['barge_collect_infos'][:]:
                ...
                if start_barge_station_id != end_barge_station_id:
                    travel_steps = generate_travel_steps_for_barge_collection(order.order_id,  tugboat.tugboat_id, start_travel_barge, 
                                                                              collection_info, order_trip, barge_ids)
            ...
            self.update_travel_info_none_order(order.start_datetime, travel_river_info)
            ...
            travel_steps = generate_travel_steps(order.order_id, tugboat.tugboat_id, arrival_datetime, travel_river_info, barge_ids, extra=" Empty Barges")
            ...
            trave_steps = generate_travel_steps(order.order_id, tugboat.tugboat_id, arrival_datetime, travel_river_info, order_trip=order_trip, 
            ...
            release_steps = generate_release_steps(order.order_id, tugboat.tugboat_id, 
                                                   release_barges_location.enter_datetime, 
                                                   order_trip, barge_ids,
                                                   appointment_info['appointment_station'])
            release_barges_location.exit_datetime = release_steps[-1].exit_datetime
            tugboat_order_results['data_points'].extend(release_steps)
            
            # if order.order_id == 'ODR_004' and tugboat.tugboat_id == 'RiverTB_01':
            #     print("Release Steps:", tugboat.tugboat_id, appointment_info['appointment_station'])
            #     raise Exception("Release Steps")
    ...
    def __update_tugboat_scheule(self, order, lookup_tugboat_results):
        self.__update_tugboat_scheule_non_order(order.order_id, lookup_tugboat_results)
    
    def __update_tugboat_scheule_non_order(self, order_id, lookup_tugboat_results, is_use_main_point = True):
        data = self.data
        order_str = ""
        for tugboat_id, tugboat_result in lookup_tugboat_results.items():
            ...
            if is_use_main_point:
                station_last = data["stations"][end_point.ID]
                
            else:
                data_point_end = tugboat_result['data_points'][-1]
            ...
            info = {
                'tugboat_id': tugboat.tugboat_id,
                'order_id': order_id if order_id != None else order_str,
    ...
    def update_single_barge_order_scheule(self, order_id, barge_id, start_datetime, end_datetime, river_km, water_status, location, station_id):
        info = {
                'barge_id': barge_id,
                'order_id': order_id,
    ...
    def update_single_barge_scheule(self, order, barge_id, start_datetime, end_datetime, river_km, water_status, location, station_id):
        self.update_single_barge_order_scheule(order.order_id, barge_id, start_datetime, end_datetime, river_km, water_status, location, station_id)
        #print("Barge info:update_single_barge_scheule", info) if barge_id == "B_084" else None
    
    
    def update_shedule_bring_down_orders_barges(self,  lookup_order_barges, lookup_tugboat_results):
        data = self.data
        ...
        for order_id, order_barge_info in lookup_order_barges.items():
            ...
            self.update_single_barge_order_scheule(order_id, barge_id, new_barge_start, max(data_point_end.exit_datetime, new_barge_start), station_last.km, 
                                             station_last.water_type, (station_last.lat, station_last.lng), data_point_end.ID)
       
        self.__update_tugboat_scheule_non_order(order_id, lookup_tugboat_results)
    ...
    def update_shedule_bring_down_barges(self, order, lookup_order_barges, lookup_tugboat_results):
        ...
        for order_id, order_barge_info in lookup_order_barges.items():
            ...
            #print("Bring down barge", barge_id) if barge_id == "B_084" else None
            self.update_single_barge_order_scheule(order.order_id, barge_id, new_barge_start, max(data_point_end.exit_datetime, new_barge_start), station_last.km, 
                                             station_last.water_type, (station_last.lat, station_last.lng), data_point_end.ID)
        ...
        for tugboat_result in tugboat_results:
            for data_point in tugboat_result['data_points']:
                ...
                if type(data_point.barge_ids) == list:
                    barge_ids = data_point.barge_ids
                else:
                    if len(data_point.barge_ids) == 0:
                        barge_ids = []
                    else:
                        barge_ids = data_point.barge_ids.replace("[", "").replace("]", "").replace("'", "").split(",") 
                
                
                barges = [self.data['barges'][barge_id] for barge_id in barge_ids]
    ...
    def calculate_full_cost_old(self, tugboat_df, barge_df=None):
        ...
        for name, group in tugboat_df_grouped:
            ...
            #check 'Start' is in group['type'] check condition
            if 'Start' in group['type'].unique():
                startStationId = group[group['type'] == 'Start']['name'].iloc[0].replace('River Start at ', '')
                startStationId = startStationId.replace('Start at ', '')
            else:
                startStationId = group[group['type'] == 'Start Collection']['name'].iloc[0].replace('River Start at ', '')
                startStationId = startStationId.replace('Start at ', '')
    ...
    def calculate_full_cost(self, tugboat_df, barge_df=None):
        ...
        for name, group in tugboat_df_grouped:
            ...
            order_ids = [name[0]]
            if "," in name[0]:
                order_ids = name[0].split(",")
            
            tugboat_id = name[1]
            order_trip = name[2]
            
            mask = ((tugboat_df['order_id'] == name[0]) &
                    (tugboat_df['tugboat_id']   == tugboat_id) &
                    (tugboat_df['order_trip'] == order_trip))
            
        
            
            tugboat = tugboats[name[1]]
            if "," in name[0]:
                order = orders[name[0].split(",")[0]]
            else:
                order = orders[name[0]]
            #print(name)
            #fixed result not assign to output_df add new row output_df 
            ...
            if 'Start' in group['type'].unique():
                startStationId = group[group['type'] == 'Start']['name'].iloc[0].replace('River Start at ', '')
                startStationId = startStationId.replace('Start at ', '')
            elif 'Travel To Carrier' in group['type'].unique() or 'Travel To Carriers' in group['type'].unique():
                #print(group)
                isTravelToCarrier = True
                items = group[group['type'] == 'Barge Step Collection']
                if len(items) > 1:
                    startStationId = items.iloc[0]['name'].split(',')[0].replace('River Start at ', '')
                    startStationId = startStationId.replace('Start at ', '')
                else:
                    print(group)
                    items = group[group['type'] == 'Barge Change Collection']
                    if len(items) == 0:
                        items = group[group['type'] == 'Barge Step Collection']
                    print(items)
                    startStationId = items['station_id'].iloc[0]
            ...
            elif 'Sea-Sea Empty Barges' in group['type'].unique():
                end_points = group[group['type'] == 'Sea-Sea Empty Barges']['name'].iloc[0].split(' to ')
                startStationId = end_points[0]
                endPointStationId = end_points[1]
                isRiverRiverLoadBarges = True
            elif 'River-Sea Empty Barges' in group['type'].unique():
                end_points = group[group['type'] == 'River-Sea Empty Barges']['name'].iloc[0].split(' to ')
                startStationId = end_points[0]
                endPointStationId = end_points[1]
                isRiverSeaEmptyBarges = True
            elif 'River-Sea' in group['type'].unique():
                strint_station = group[group['type'] == 'River-Sea']['name'].iloc[0]
                end_points = strint_station.split(' to ')
                if 'at' in strint_station:
                    end_points = strint_station.split(' at ')
                    startStationId = end_points[1]
                    endPointStationId = end_points[1]
                else:
                    startStationId = end_points[0]
                    endPointStationId = end_points[1]
                isRiverSeaEmptyBarges = True
            elif 'River-River' in group['type'].unique():
                end_points = group[group['type'] == 'River-River']['name'].iloc[0].split(' to ')
                startStationId = end_points[0]
                end_points = group[group['type'] == 'River-River']['name'].iloc[-1].split(' to ')
                endPointStationId = end_points[1]
                ...
                if tugboat.get_type() == WaterBody.RIVER:
                    ...
                    #regex to find station id by get string in ()
                    # define regex pattern
                    pattern = r'\((.*?)\)'
                    # find all matches
                    matches = re.findall(pattern, string_station)
                    # get first match
                    startStationId = matches[0].split(' to ')[0]
                    isRiverTugboat = True
                else:
                    # print(group['type'].unique())
                    # print(group)
                    startStationId = group[group['type'] == 'Start Collection Carrier']['name'].iloc[0].replace('River Start at ', '')
                    startStationId = startStationId.replace('Start at ', '')
            
            appointment = group[group['type'] == 'Appointment']['ID']
          
            if isRiverRiverLoadBarges or isRiverSeaEmptyBarges or isTravelToCustomer or isRiverRiverEmptyBarges or isRiverRiver:
                pass
                ...
                ids = group[group['type'] == 'Barge Change']['ID']
                ids2 = group[group['type'] == 'Barge Release']['name']
                if isTravelToCarrier:
                    endPointStationId = ids2.iloc[0].split(' ')[-1]
                elif isTravelToCustomer:
                    pass
                elif isRiverTugboat:
                    # if len(order_ids) > 1:
                    #     raise Exception("Order ID has multiple orders")
                    pass
                elif len(ids) > 0:
                    endPointStationId = ids.iloc[0]
                else:
                    #print(group)
                    startPointStationId = group[group['type'] == 'Start']['name'].iloc[0].split(' ')[-1]
            ...
            parkingTime = group[group['name'].str.contains('stop at')]['rest_time'].sum()
            #time_move = 0
            havy_time_move = group[(group['type'] == 'Travel To Customers') | (group['type'] == 'Carrier Station') |
                                  (group['type'] == 'Appointment') | 
                                  (group['type'] == 'Travel To Carrier') ]['time'].sum()
            soft_time_move = group[(group['type'] == 'Barge Collection') | 
                                  (group['type'] == 'Travel To Carrier') | 
                                  (group['type'] == 'Barge Change')]['time'].sum()
            
            # check is not defined
            # if startPointStationId is None:
            #     print(group['type'].unique())
            #     print(group)
            total_load = 0
            distance =0
            ...
            items = group[(group['type'] == 'Appointment')]
            total_load += items['total_load'].sum()
            
            
            is_havy_move = len(items) != 0
            key_name = "Appointment" if len(items) != 0 else None
            barge_ids = items['barge_ids'].iloc[0] if len(items) != 0 else None
            distance = items['distance'].iloc[0]  if len(items) != 0 else 0
            
            
            if order.order_type == TransportType.IMPORT and len(items) == 0:
                items = group[(group['type'] == 'Travel To Customer')]
                barge_ids = items['barge_ids'].iloc[0] if len(items) != 0 else None
                if  "," in name[0]:
                    total_load += items['total_load'].iloc[0] if len(items) != 0 else 0
                else:
                    total_load += items['total_load'].sum()
                
                key_name = "Travel To Customer" if len(items) != 0 else None
                distance = items['distance'].iloc[0]  if len(items) != 0 else 0
            
            elif order.order_type == TransportType.EXPORT:
                items = group[(group['type'] == 'Travel To Carrier')]
                if  "," in name[0]:
                    total_load += items['total_load'].iloc[0] if len(items) != 0 else 0
                else:
                    total_load += items['total_load'].sum()
                key_name = "Travel To Carrier" if len(items) != 0 else None
                barge_ids = group[(group['type'] == 'Travel To Carrier')]['barge_ids'].iloc[0] if len(items) != 0 else None
                distance = items['distance'].iloc[0]  if len(items) != 0 else 0
    ...
    def calculate_full_barge_cost_old_v2(self, tugboat_df):
        ...
        #filter tugboat_df by type that contain Loader-Carrier
        tugboat_df_loader_carrier = tugboat_df[tugboat_df['type'].str.contains('Loader-Customer')]
        #print(tugboat_df_loader_carrier)
            ...
            for order_id in order_ids:
                ...
                if len(appointment) > 0:
                    ...
                    if len(ids) > 0:
                        ...
                        data_log2 = group[(group['type'] == 'River-River Load Barges') | (group['type'] == 'River-Sea Empty Barges') 
                                          | (group['type'] == 'River-Sea') | (group['type'] == 'River-River')]
                        ...
                        elif len(data_log2) > 0:
                            object_element = data_log2.iloc[0]
                        elif len(data_log3) > 0:
                            object_element = data_log3.iloc[0]
                        elif len(data_log4) > 0:
                            object_element = data_log4.iloc[0]
                        else:
                            object_element = group[group['type'] == 'Barge Release'].iloc[0]
                        #total_load_barge = object_element['total_load']
                ...
                if startPointDatetime is pd.NaT:
                    if order.order_type == TransportType.IMPORT:
                        startPointDatetime = group[group['type'] == 'Start Collection Carrier']['enter_datetime'].min()
                    else:
                ...
                if startPointDatetime is pd.NaT:
                    if order.order_type == TransportType.IMPORT:
                        startPointDatetime = group[group['type'] == 'Barge Change Collection']['enter_datetime'].min()
                    else:
                        startPointDatetime = group[group['type'] == 'Start Collection Customer']['enter_datetime'].min()    
                if startPointDatetime is pd.NaT:
                    if order.order_type == TransportType.IMPORT:
                        startPointDatetime = group[(group['type'] == 'River-River Load Barges') | 
                                                   ...
                                                   (group['type'] == 'River-Sea') | 
                                                   (group['type'] == 'River-River')]['enter_datetime'].min()
                ...
                #print("barge_ids", barge_ids, object_element)
                ...
                for barge_id in barge_ids: 
                    ...
                    if len(items) > 0:
                        ...
                        if len(items) > 0:
                            ...
                            if len(items) == 0:
                                items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Change')]
                            if len(items) == 0:
                                items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Step Release')]
                                
                            if len(items) == 0:
                                items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Step Collection')]
                            if len(items) == 0:
                                items = group[(group['name'].str.contains(barge_id)) & (group['type'] == 'Barge Step Collection')]
                                if len(items) == 0:
                                    #print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
                            ...
                            startDatetime = items['enter_datetime'].iloc[0]
                            startStationId = items['name'].iloc[0].split(' ')[-1].replace(')', '')
                    finishDatetime = tugboat_df_loader_carrier[tugboat_df_loader_carrier['name'].str.contains(barge_id)]['exit_datetime'].max()
                    isFinishDatetime = True       
                    ...
                    if len(items) > 0:
                        ...
                        items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Change Collection')]
                        if len(items) == 0:
                            items = group[(group['name'].str.contains(barge_id)) & (group['type'] == 'Barge Step Collection')]
                        if len(items) == 0:
                            items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Step Collection')]
                        if len(items) == 0:
                            items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Change')]
                        if len(items) == 0:
                            items = group[(group['name'].str.contains(barge_id)) & (group['type'] == 'Barge Collection')]
                        if len(items) == 0:
                            items = group[(group['barge_ids'].str.contains(barge_id)) & (group['type'] == 'Barge Collection')]
    ...
    def calculate_full_barge_cost(self, tugboat_df, cost_df):
        #group barge_df by barge_id and order_id and order_trip
        
        # all_barges = tugboat_df[tugboat_df['type'] == 'Loader-Customer']
        # print("Total Customer-Loader ###############################", all_barges["total_load"].sum())
        all_barges = tugboat_df[tugboat_df['type'] == 'Crane-Carrier']
        lookup_order_id_barges= {}
        for i in range(len(all_barges)):
            row_data = all_barges.iloc[i]
            barge_id = row_data["barge_ids"]
            order_id = row_data["order_ids"]
            key = order_id+"_"+barge_id
            key.strip()
            lookup_order_id_barges[key] = row_data['total_load']
            #print(key, row_data['name'])
            
        
        
        #print("lookup_order_id_barges", lookup_order_id_barges)
        
        #print("Total barge ###############################", all_barges[all_barges['order_id'] == 'ODR_002']["total_load"].sum())
        
        #group all_barges by order_id 
        
        
        name_order = "ODR_002"
        order_ids_df = cost_df[cost_df['OrderId'].str.contains(name_order)]
        total_load = 0
        #print(order_ids_df)
        
        for i in range(len(order_ids_df)):
            row_data = order_ids_df.iloc[i]
            if not row_data['BargeIds'] :
                continue 
            if "River"  in row_data['TugboatId']:
                continue
            
            barge_ids = row_data['BargeIds'].split(",")
            for bid in barge_ids:
                key = name_order+"_"+bid.strip()
                #print(key)
                if key in lookup_order_id_barges:
                    total_load += lookup_order_id_barges[key]
                # else:
                #     print(row_data)
                    #raise Exception("Key not found", key)
                
        #print(f"{name_order} Total barge #########`######################", total_load)
        ...
        all_barges_grouped = all_barges.groupby(['order_id'], as_index=False)
        
        order_ids =  np.unique(cost_df['OrderId'])
        
        
        for order_id, group in all_barges_grouped:
            # print("Order", order_id)
            # print(group['total_load'].sum())
        
            total_v2 = 0
            total_v3 =0 
            for i in range(len(group)) :
                row_data = group.iloc[i]
                load_barge = row_data['total_load']
                barge_id = row_data['barge_ids']
                order_id = row_data['order_ids']
                #print(barge_id, load_barge, order_id)
                total_v2 += load_barge
                
                group_barge = cost_df[cost_df['BargeIds'].str.contains(barge_id) & cost_df['OrderId'].str.contains(order_id)]
                
                #if len(group_barge) == 0:
                #    print("EEEEEEEEEEAEEEEEEEEEEEEEEEEEEEEEE No group barge", barge_id, order_id) 
                
                
                for j in range(len(group_barge)):
                    row_tugboat_data = group_barge.iloc[j]
                    #order_id = row_tugboat_data['OrderId']
                    # if ',' in order_id:
                    #     print(row_tugboat_data)
                        #raise Exception("Order id contains comma")
                    
                    
                    #print(row_tugboat_data)
                    if row_tugboat_data['Cost'] != 0 and  "Sea" in row_tugboat_data['TugboatId']:
                        total_v3 += load_barge
                    output_df = output_df._append({
                        "BargeId": barge_id,
                        "TugboatId": row_tugboat_data['TugboatId'],
                        "OrderId": order_id,
                        "Time": row_tugboat_data['Time'],
                        "Distance": row_tugboat_data['Distance'],
                        "Cost": row_tugboat_data['Cost'],
                        "Load": load_barge,
                        "StartDatetime": row_tugboat_data['StartDatetime'],
                        "StartPointDatetime": row_tugboat_data['StartPointDatetime'],
                        "FinishDatetime": row_tugboat_data['FinishDatetime'],
                        "StartStationId": row_tugboat_data['StartStationId'],
                        "StartPointStationId": row_tugboat_data['StartPointStationId'],
                        "EndPointStationId": row_tugboat_data['EndPointStationId'],
                        "UnloadLoadTime": row_tugboat_data['UnloadLoadTime'],
                        "ParkingTime": row_tugboat_data['ParkingTime'],
                        "MoveTime": row_tugboat_data['MoveTime'],
                        "OrderTrip": row_tugboat_data['OrderTrip'],
                        "AllTime": row_tugboat_data['AllTime'],
                    }, ignore_index=True)
                    #break
                    
                    #print(order_id, barge_id, load_barge, row_tugboat_data['TugboatId'], row_tugboat_data['BargeIds'])
            #print(total_v2, total_v3)
        
            
        return output_df
    

    def __display_table_order_info(self, order_ids):
        data = self.data
        barges = data['barges']
        orders = data['orders']
        remaining_load_demand_order_ids = {}
        total_rate_hours = {}
        # print header table order_id, order.demand, order.get_total_rate_hours()
        print( "| Order Id |  Demand  |Total Rate Hours|")
        for order_id in order_ids:
            order = orders[order_id]
            remaining_load_demand_order_ids[order_id] = order.demand
            #adjust space column put varialbe print as row table order_id, order.demand, order.get_total_rate_hours() column aligment space ระหว่าง column
            # adjust the priint to match header column space each column
            order_id_space = len(order_id) + 1
            demand_space = 8
            total_rate_hours_space = 15
            
            print(f"| {order_id:{order_id_space}} | {order.demand:{demand_space}} | {order.get_total_rate_hours():{total_rate_hours_space}} | {order.order_type}")
        print("--------------------------------", sum(remaining_load_demand_order_ids.values()))
    
    def __display_timeline(self, order_ids, start_date, end_date, load_order_checks):
        # Create date list
        date_list = []
        ...
        for order_id in order_ids:
            ...
            print(f"{order_id} {order.demand} {order_start} {order_end} {load_order_checks[order_id]}")
    ...
    def _bring_barges_orders_travel(self, assigned_barges, order_trip, is_import=True):
        """
        Unified method to handle both import and export barge travel.
        ...
        for tugboat_id in tugboats:
            tugboats[tugboat_id].reset()
        
        orders = self.data['orders']
        tugboat_results = []
        iteration = 0
        barges = [bargeinfo['barge'] for bargeinfo in assigned_barges]
        all_barge_ids = [bargeinfo['barge'].barge_id for bargeinfo in assigned_barges]
        #print("Len Ids", len(all_barge_ids))
        lookup_assigned_barges = {bargeinfo['barge'].barge_id: bargeinfo['assigned_order'] for bargeinfo in assigned_barges}
        while len(barges ) > 0:
            #print("Barges remain:", len(barges))
            ...
            while not is_completed and iteration < 100:
                is_completed, assigned_tugboats = self.assign_barges_to_tugboats_non_order(order, order.start_object.station, 
                                                                                    order.start_datetime, order.due_datetime, 
                                                                                    tugboats, barges, travel_before_days=travel_before_days)
                iteration += 1
                if iteration > 100:
                    break
                if not is_completed:
            ...
            for tugboat in assigned_tugboats:
                barge_ids = [barge.barge_id for barge in tugboat.assigned_barges ]
                #print(barge_ids, lookup_assigned_barges)
                #print(all_barge_ids)
                order_ids = [lookup_assigned_barges[barge_id] for barge_id in barge_ids]
            ...
            # Process barge travel
            barge_tugboat_results, late_time = self.arrival_step_travel_empty_barges(order, assigned_tugboats, 
                                                                                     appointment_info, order_trip,
                                                                                     is_startdate_order=True)
            if len(barge_tugboat_results) > 10:
                print("Debug Error")
                for tugboat_result in barge_tugboat_results:
                    print(tugboat_result)
                raise Exception("No tugboat results", len(barge_tugboat_results))
    ...
    def _bring_barge_orders_travel_import(self, assigned_barges, order_trip, is_import=True):
        return self._bring_barges_orders_travel( assigned_barges, order_trip, is_import)
    
    def arrival_step1_barges_orders_to_appointment(self, assigned_barges, is_do_import):
        ...
        if len(bring_down_river_barges) == 0:
            ...
            #print("Bring down river barge", order.order_id, [barge.barge_id for barge in bring_down_river_barges])
            #print("Bring down river barge", order.order_id, [self.barge_scheule[barge.barge_id][-1]['station_id'] for barge in bring_down_river_barges])
            #barge_ids = [barge['barge'].barge_id for barge in bring_down_river_barges]
            #print("DEBUG:", len(bring_down_river_barges), len(assigned_barges), barge_ids)
            isCompleted, tugboat_results = self._bring_barge_orders_travel_import( bring_down_river_barges, 
                                                                                  order_trip=1, is_import=True)
            if not isCompleted:
                if len(tugboat_results) > 10:
                    
                    # for tugboat_result in tugboat_results:
                    #     print(tugboat_result)
                    print("Debug Error")
                raise Exception("No tugboat results", len(tugboat_results))
                
                
                raise Exception("Bring down river barge failed", len(bring_down_river_barges), len(tugboat_results))
                print("Bring down river barge failed", len(bring_down_river_barges), len(tugboat_results))
                return None, tugboat_results
            lookup_tugboat_results = {tugboat_result['tugboat_id']: tugboat_result for tugboat_result in tugboat_results}
            ...
            order_barges, lookup_order_barges = order_barges_from_arrival_tugboats(data, lookup_tugboat_results)
            self.update_shedule_bring_down_orders_barges( lookup_order_barges,
                                                    lookup_tugboat_results)  
            self._extend_update_tugboat_results(tugboat_results, 0)
            self._reset_all_tugboats()
    ...
    def arrival_step_transport_all_orders(self, min_order_id, start_station, min_start_datetime,
                                                max_due_datetime, assigned_tugboats, assigned_barge_order_ids, order_trip = 1, is_export=False):
        time_boat_lates = []
        tugboat_results = []
        arrival_times = []
        for tugboat in assigned_tugboats:
            ...
            collection_time_info = tugboat.calculate_collection_barge_time(tugboat_info, self.barge_scheule)
            self.update_collection_barge_time_none_order(min_start_datetime, collection_time_info)
            
            
            # orders = []
            # for barge in tugboat.assigned_barges:
            #     order_id = assigned_barge_order_ids[barge.barge_id]
            #     orders.append(self.data['orders'][order_id])
            ...
            #print("Travel info", travel_info)
            self.update_travel_info_none_order(min_start_datetime, travel_info)
            #print("Travel info updated", travel_info)
            #raise Exception("Travel info updated")
            ...
            str_orders = []
            min_start_order = None
            orders = self.data['orders']
            for barge in tugboat.assigned_barges:
                barge_id = barge.barge_id
                order_id = assigned_barge_order_ids[barge_id]
                if min_start_order is None or orders[order_id].start_datetime > min_start_order:
                    min_start_order = orders[order_id].start_datetime 
                if order_id in str_orders:
                    continue
                str_orders.append(order_id)
            
            target_datetime = min_start_order + timedelta(days=NEXT_DAY_WORK)
            if tugboat_ready_time < target_datetime:
                tugboat_ready_time = target_datetime
                
                
            
            start_pos = tugboat_info['station_id'] 
            start_pos = tugboat_info['location'] if start_pos == None else start_pos  
            str_order_ids = str_orders[0] if len(str_orders) == 1 else ', '.join(str_orders)
            start_location = DataPoint(
                ID = "Start",
                type = "Start",
                name = "Start at " + start_pos,
        ...
        # print(total_time, travel_info)
            ...
            if order_trip == 1:
                if arrival_time < min_start_datetime:
                    arrival_time = min_start_datetime
                else:
                    time_lated_start = (arrival_time - min_start_datetime).total_seconds() / 3600
            ...
            for i, collection_info in enumerate(collection_time_info['barge_collect_infos']):# collection_time_info['barge_collect_infos'][:]:
                ...
                str_order  = assigned_barge_order_ids[barge_id]
                barge_step = DataPoint(
                    ID= "Barge",
                    type= "Barge Step Collection",
                    name= name,
        ...
        #print("Arrival times:", arrival_times)
            ...
            travel_info = tugboat.calculate_travel_to_multiple_start_objects( self.barge_scheule)
            
            trave_steps = generate_travel_steps(str_order_ids, tugboat.tugboat_id, arrival_datetime, travel_info,  order_trip, barge_ids,
                                            extra=" Empty Barges")
            
            
            ID_NAME = "Travel To Carrier" if not is_export else "Travel To Customer"
            
            
            rest_time = sum([point.rest_time for point in trave_steps])
            travel_to_carrier = DataPoint(
                ID=f"{ID_NAME}s",
                type=ID_NAME,
                name=str_order_ids,
                enter_datetime=arrival_datetime,
                ...
                barge_ids= barge_ids,
                order_trip = order_trip,
                station_id = self.data['orders'][str_orders[0]].start_object.station.station_id,
    ...
    def update_shedule_tugboats_barges(self, lookup_order_barges, lookup_tugboat_results, is_use_main_point=False, lookup_target_stations=None) :
        data = self.data
    
        for order_id, order_barge_info in lookup_order_barges.items():
            barge_id = order_barge_info['barge_id']
            #print("Update Barge", barge_id)
            tugboat_result = self._find_tugboat_result(barge_id, lookup_tugboat_results)
            ...
            #print("Bring down barge", barge_id) if barge_id == "B_084" else None
            self.update_single_barge_order_scheule(target_order_id, barge_id, new_barge_start, max(data_point_end.exit_datetime, new_barge_start), station_last.km, 
                                             station_last.water_type, (station_last.lat, station_last.lng), station_id)
       
        self.__update_tugboat_scheule_non_order(None, lookup_tugboat_results, is_use_main_point)
    
    def arrival_step2_barges_orders_to_start_points(self, assigned_barges, assigned_barge_order_ids, is_do_import):
        if not is_do_import:
            print("Skip Export")
            return None, None
        ...
        for tugboat_id in tugboats:
            tugboats[tugboat_id].reset()
        all_assigned_barges = [barge_info['barge'] for barge_info in assigned_barges]
        lookup_assigned_barges = {barge_info['barge'].barge_id: barge_info for barge_info in assigned_barges}
        order_ids = [barge_info['assigned_order'] for barge_info in assigned_barges]
        min_order_id = None
        max_due_datetime = max([orders[order_id].due_datetime for order_id in order_ids])
        ...
        while len(all_assigned_barges) > 0:
            ...
            if not is_completed or iteration > 100:
                print("Step 2 Not completed here assign barge to tugboat", order_ids, len(all_assigned_barges))
                raise Exception("Step 2 Not completed here assign barge to tugboat", order_ids, len(all_assigned_barges))
                return False, {
            ...
            tugboat_results, late_time = self.arrival_step_transport_all_orders(min_order_id, start_station, min_start_datetime,
                                                                                max_due_datetime, assigned_tugboats, assigned_barge_order_ids, order_trip=round_trip_order)
            tugboat_ids = [tugboat_result['tugboat_id'] for tugboat_result in tugboat_results]
            
            
            for tugboat_result in tugboat_results:
                tugboat_id = tugboat_result['tugboat_id']
                tugboat = self.data['sea_tugboats'][tugboat_id]
                barge_ids = [barge.barge_id for barge in tugboat.assigned_barges]
                #print(" Step2 Tugboat Result", tugboat_id, barge_ids)
                for barge in tugboat.assigned_barges:
                    barge_id = barge.barge_id
            ...
            all_assigned_barges = copy_all_assigned_barges
            #print("Order Trip", round_trip_order, "Tugboat IDS", tugboat_ids, len(tugboat_ids))
            round_trip_order += 1
    ...
    def arrival_step_transport_orders_to_appointment(self, assigned_tugboats, assigned_barge_order_ids, order_trip):
        data = self.data
        tugboat_results = []
        time_boat_lates = []
        arrival_times = []
        for tugboat in assigned_tugboats:
            ...
            barge_ids = [barge.barge_id for barge in tugboat.assigned_barges]
            station_barge_ids = [self.get_station_id_barge(barge) for barge in tugboat.assigned_barges]
            last_barge_info = self.barge_scheule[barge_ids[-1]][-1]
            previos_datetime = last_barge_info['end_datetime'] - timedelta(days=2)
            if previos_datetime > tugboat_ready_time:
                previos_datetime = tugboat_ready_time
            
            self.update_collection_barge_time_none_order(previos_datetime, collection_time_info)
            
            # print("Debug Collection Time ############################")
            # for barge_collect_info in collection_time_info['barge_collect_infos']:
            #     travel_steps = barge_collect_info['travel_steps']
            #     print("Travel step", tugboat_id)
            #     for travel_step in travel_steps:
            #         print("\t",travel_step.start_id, travel_step.end_id, travel_step.start_time, travel_step.exit_time, barge_collect_info['barge_id'])
            #     #print(barge_info)
            ...
            #print("Travel info", travel_info)
            self.update_travel_info_none_order(previos_datetime, travel_info)
            ...
            barge_ready_time = self.get_ready_barge(self.data["barges"][barge_ids[0]])
            barge_ready_time = get_next_quarter_hour(barge_ready_time)
            start_barge_station_id = first_barge_location['travel_steps'][0].start_id
        ...
        # print(total_time, travel_info)
            ...
            if order_trip == 1:
                if arrival_time < previos_datetime:
                    arrival_time = previos_datetime
                else:
                    time_lated_start = (arrival_time - previos_datetime).total_seconds() / 3600
        ...
        #print("Arrival times:", arrival_times)
            ...
            barge_ids = [barge.barge_id for barge in tugboat.assigned_barges]
            appointment_location = DataPoint(
                ID=appointment_station.station_id,
                ...
                barge_ids= barge_ids,
                order_trip = order_trip,
                station_id = travel_info['start_object'].station.station_id,
                order_ids = str_order_ids,
                tugboat_id = tugboat.tugboat_id
            )
            ...
            release_barges_location.exit_datetime = appointment_location.exit_datetime + timedelta(minutes=time_release_barges)
            tugboat_order_results['data_points'].append(release_barges_location)
            release_steps = generate_release_steps(str_order_ids, tugboat.tugboat_id, release_barges_location.enter_datetime, order_trip,
                                                barge_ids, appointment_station.station_id)
            release_barges_location.exit_datetime = release_steps[-1].exit_datetime
            tugboat_order_results['data_points'].extend(release_steps)
            
            for i, release_step in enumerate(release_steps):
                order_id = assigned_barge_order_ids[barge_ids[i]]
    ...
    def arrival_step3_barges_orders_to_appointment(self, assigned_barges,  assigned_barge_order_ids, lookup_order_barges, 
                                                        lookup_order_crane_infos, is_do_import, round_order_trip=1):
        if not is_do_import:
            print("Skip Export")
            return None, None
        ...
        #     total_load += barge_info['barge'].get_load(True)
        #     if barge_info['barge'].barge_id == 'B_100':
        #         print("### B_100", barge_info['barge'].get_load(True))
        ...
        #print("Schedule Barges DEBUG:", len(barge_schedules))
        arrived_barges = []
        for barge_schedule in barge_schedules:
            barge_id = barge_schedule['barge_id']
            barge_info = lookup_barge_schedules[barge_id]
            order = orders[barge_info['assigned_order']]
            #print(barge_id, barge_schedule['delta_time'], barge_schedule['time_consumed'],  
                  #barge_schedule['start_datetime'], barge_schedule['end_datetime'])
            arrived_barges.append(lookup_barge_schedules[barge_id])
            station = order.start_object.station
            self.update_single_barge_scheule(order, barge_id, barge_schedule['start_datetime'], barge_schedule['end_datetime'], 
        ...
        for barge_schedule in barge_schedules:
            ...
            #print(active_crane_info)
            active_crane_info = lookup_crane_info[crane_id]
            #print(active_loader_info)
            ...
            crane_location = DataPoint(
                ...
                #exit_datetime=barge_schedule['end_datetime'],
                distance=0,
            ...
            schedule_results.append(crane_location) # add result data points
        ...
        iteration = 0
        round_trip_order = round_order_trip
        all_tugboat_results = []
        all_lookup_order_barges = {}
        
        total_load = 0
        for barge in all_assigned_barges:
            total_load += barge.get_load(True)
            if barge.barge_id == 'B_100':
                print("### B_100", barge.get_load(True))
        
        print("### all_assigned_barges Total Load", len(all_assigned_barges), total_load)
        ...
        while len(all_assigned_barges) > 0:
            ...
            while not is_completed:
                ...
                if not is_completed:
                    travel_before_days += 5
                    #print("Not completed here assign barge to tugboat =================================================================", travel_before_days)
            
            if not is_completed or iteration > 100:
            
                print("Tugboats ready time ######################################################################")
                raise Exception("Step 3 Tugboats ready time ######################################################################", len(all_tugboat_results))
                for tugboat in tugboats.values():
            ...
            for tugboat in assigned_tugboats:
                ...
                total_load = sum(b.get_load(True) for b in tugboat.assigned_barges)
                print("\tTugboat", tugboat_id, "Total Load", total_load, "Barges", barges_ids)
                #update tugboat result
            
            
            #debug total load barges 
            total_load = 0
            for tugboat in assigned_tugboats:
                total_load += sum(b.get_load(True) for b in tugboat.assigned_barges)
            print("############### Step 3 Load Tugboat", tugboat.tugboat_id, "Total Load", total_load)
            
            
            tugboat_results, late_time = self.arrival_step_transport_orders_to_appointment( assigned_tugboats, assigned_barge_order_ids, 
            ...
            for tugboat_result in tugboat_results:
                ...
                total_load += sum(b.get_load(True) for b in tugboat.assigned_barges)
            print("############### Step 3 v2 Load Tugboat", tugboat.tugboat_id, "Total Load", total_load)
                
            before_len = len(all_assigned_barges)
            for barge in all_approved_barges:
                # remove barge in all_assigned_barges where the same barge_id
                for barge_info in all_assigned_barges:
                    if barge_info.barge_id == barge.barge_id:
                        all_assigned_barges.remove(barge_info)
                        break
            if len(all_assigned_barges) != len(copy_all_assigned_barges):
                raise Exception("All assigned barges", before_len, len(all_assigned_barges), len(copy_all_assigned_barges), len(all_approved_barges))
            ...
            lookup_tugboat_results = {tugboat_result['tugboat_id']: tugboat_result for tugboat_result in tugboat_results}
            order_barges, lookup_order_barges = order_barges_from_arrival_tugboats(self.data, lookup_tugboat_results)
            self.update_shedule_tugboats_barges( lookup_order_barges, lookup_tugboat_results, True, lookup_target_stations)    
            self._extend_update_tugboat_results(tugboat_results, round_trip_order)       
            self._reset_all_tugboats()
        ...
        #order finish barge and assigned to tugboat
    
    def arrival_step_transport_orders_to_end_points(self, assigned_tugboats, assigned_barge_order_ids, 
                                                    lookup_barge_infos, round_order_trip, is_export=False):
        time_boat_lates = []
        tugboat_results = []
        arrival_times = []
        ...
        #     # Update data points with calculated times
            ...
            temp_barge_ids = []
            for collection_info in collection_time_info['barge_collect_infos'][:]:
                if collection_info['barge_id'] not in temp_barge_ids:
            ...
            arrival_datetime = start_location.enter_datetime
            
            #start_station
            
            # if "ODR_003" in str_order_ids and tugboat.tugboat_id == 'RiverTB_01':
            #     print("Travel Inof", travel_info)
            #     raise Exception("Debug")
            
            travel_steps = generate_travel_steps(str_order_ids, tugboat.tugboat_id, 
                                                 arrival_datetime, travel_info,  round_order_trip, [],
                                             extra=" Empty Barges")
            #barge_location.enter_datetime is last step travel
            travel_steps = None
            # start_barge_station_id = collection_info['travel_steps'][0].start_id
            # end_barge_station_id = collection_info['travel_steps'][-1].end_id
            ...
            #                                                                 collection_info, 
            #                                                                 str_order_ids, [])
            #     print("Travel steps", tugboat.tugboat_id, order_id, start_barge_station_id, end_barge_station_id)
                
            #     raise Exception("Debug")

            tugboat_result = {
                'tugboat_id': tugboat.tugboat_id,
                'data_points': [start_location]
            }
            tugboat_results.append(tugboat_result)
            # if travel_steps != None: 
            #     tugboat_result['data_points'].extend(travel_steps)
            #     barge_location.enter_datetime = travel_steps[-1].exit_datetime
            #     start_travel_barge = barge_location.enter_datetime
            tugboat_result['data_points'].append(barge_location)
            ...
            for collection_info in collection_time_info['barge_collect_infos'][:]:
                ...
                barge_id = collection_info['barge_id']
                barge_point_schedule = self.barge_scheule[barge_id][-1]
                #print(barge_point_schedule)
                if start_travel_barge < barge_point_schedule['end_datetime']:
                    start_travel_barge = barge_point_schedule['end_datetime']    
                ...
                if start_barge_station_id != end_barge_station_id:
                    #print("Travel steps", start_barge_station_id, end_barge_station_id, collection_info)
                    travel_steps = generate_travel_steps_for_barge_collection(order_id,  
                                                                              tugboat.tugboat_id, 
                                                                              start_travel_barge, 
                                                                              collection_info, 
                                                                              round_order_trip, barge_ids)
                    barge_steps.extend(travel_steps)
                    start_travel_barge = travel_steps[-1].exit_datetime
                ...
                order_id = assigned_barge_order_ids[collection_info['barge_id']]
                if order_id not in order_ids:
                    order_ids.append(order_id)
                str_order_ids = ", ".join(temp_order_ids)
                #print(collection_info)
                barge_step = DataPoint(
                    ID= "Barge",
                    type= "Barge Change Collection",
            ...
            #print("Barge Step Release", temp_barge_ids)
            ...
            #sorted str_orders base on km
            str_orders = sorted(str_orders, key=lambda x: self.data['orders'][x].des_object.station.km)
        
            
            ID_NAME = "Travel To Customer" if not is_export else "Travel To Carrier"
            ...
            release_steps = generate_release_steps(str_order_ids, tugboat.tugboat_id, release_barges_location.enter_datetime, 
            ...
            if len(travel_infos) > 1:
                ...
                for i in range(1, len(travel_infos)):
                    ...
                    barge_ids = [barge.barge_id for barge in tugboat.assigned_barges if barge.current_order.order_id == str_orders[i]]
                    ...
                    travel_steps = generate_travel_steps(str_order_ids, tugboat.tugboat_id, arrival_datetime, travel_info,  round_order_trip, 
                                                            barge_ids, extra=" Load Barges")
                    
                    
                    
                    station_id = self.data['orders'][str_orders[i]].des_object.station.station_id
                    release_barges_location = DataPoint(
                            ID="Release Barges",
                            type="Barge Release",
                            name="Release Barges (" + " - ".join(barge_ids) + ")",
            ...
            if len(str_orders) > len(travel_infos):
                #print("Travel To Multiple Carrier Barge ids", tugboat.tugboat_id, barge_ids)
                for i in range(1, len(travel_infos )):
                    #print("Travel To Multiple Carrier Barge ids", tugboat.tugboat_id, barge_ids)
                    barge_ids = [barge.barge_id for barge in tugboat.assigned_barges if barge.current_order.order_id == str_orders[i]]
                    ...
                    release_steps = generate_release_steps(str_order_ids, tugboat.tugboat_id, release_barges_location.enter_datetime, 
                                                        round_order_trip, temp_barge_ids    , station_id)
                    release_barges_location.exit_datetime = release_steps[-1].exit_datetime
            ...
            # if 'B_041' in barge_location.barge_ids:
            #     print("DEBUG B_041", station_id, tugboat.tugboat_id, barge_location.barge_ids, len(barge_steps),
            #         str_order_ids, [barge.barge_id for barge in tugboat.assigned_barges ])
            #     raise Exception("B_041")
    ...
    def arrival_step4_transport_orders_to_end_points(self, assigned_barges,  assigned_barge_order_ids, global_lookup_order_barges, 
        ...
        while len(all_assigned_barges) > 0:
            ...
            while not is_completed:
                is_completed, assigned_tugboats = self.assign_barges_to_tugboats_non_order(min_order_id, start_station, min_start_datetime,
                                                                                       max_due_datetime, tugboats, copy_all_assigned_barges, next_day)
                next_day += 5
                iteration_sub += 1
                if iteration_sub > 5:
                    raise Exception("Step 4 Not completed here assign barge to tugboat", order_ids, len(all_assigned_barges), len(assigned_tugboats))
                if not is_completed:
                    print("Repeated next days step 4 #######################", next_day)
            
                
            if not is_completed or iteration > 100:
                print("Step 4 Not completed here assign barge to tugboat", order_ids, len(all_assigned_barges))
                raise Exception("Step 4 Not completed here assign barge to tugboat", order_ids, len(all_assigned_barges), len(assigned_tugboats))
                return False, {
                              'assign_barge_infos': assigned_barges,
                              'assign_river_barges':[],
            ...
            if len(assigned_tugboats) == 0:
                raise Exception("No tugboat found for order: " + str(order.order_id), len(all_assigned_barges), len(all_tugboat_results))
                #print("Order: {} No tugboat found".format(order.order_id))
                break
            # print("Assigned Tugboats: ###############################", len(assigned_tugboats))
    ...
    def schedule_step5_customer_loading_barges(self, lookup_order_barges, lookup_order_loading_infos):
        schedule_results = []
        barge_schedules = []
        lookup_barge_schedules = {}
        orders = self.data['orders']
        ...
        for order_id, barge_infos in lookup_order_barges.items():
            ...
            #print("Schedule Step 5 Customer Loading Barges", active_loader_info)
            shedule_result = schedule_customer_order_barges(self, order, barge_infos, active_loader_info)
            barge_schedules.extend(shedule_result['barge_schedule'])
        ...
        for barge_schedule in barge_schedules:
            ...
            schedule_results.append(loader_location) # add result data points
    ...
    def assign_barges_single_window(self, remaining_orders, assigned_orders, remaining_load_demand_order_ids, 
                                        start_travel_datetime, target_travel_datetime, next_orders, is_do_import):
        MAX_ORDER_DEMAND = 50000
        TOTAL_MAX_DEMAND = 100000
        EXPECT_DAYS = 5
        MEAN_RATE = 2000
        MAX_ORDER_SINGLE_WINDOW = 3
        
        orders = self.data['orders']
        barges = self.data['barges']
        ...
        order_ids = list(remaining_orders)
        #sorted based on ration of due time - start_travel_datetime  and remaining demand
        # start_travel_datetime is datetime.date
        # orders[x].due_datetime is datetime.datetime
        # convert start_travel_datetime to datetime.datetime
        converted_start_travel_datetime = datetime.combine(start_travel_datetime, datetime.min.time())
        
        
        all_order_min_start_datetime = min([orders[x].start_datetime for x in order_ids])
        
        def sort_func(x):
            # demand_factor = remaining_load_demand_order_ids[x]
            # time_factor = ((orders[x].due_datetime - converted_start_travel_datetime) + (orders[x].start_datetime - all_order_min_start_datetime)).total_seconds() / 3600 
            scale_factor = 1
            if x in next_orders:
                scale_factor = 0.05
            #scale_factor = 1
        ...
        order_ids.sort(key=sort_func)
        
        #print("Sorted Remaining Order IDs", order_ids)
        ...
        for order_id in order_ids:
            ...
            if start_travel_datetime <= order.start_datetime.date()  and order.start_datetime.date() <= target_travel_datetime:
                #delta time
                ...
                delta_time = EXPECT_DAYS*24
                current_window_orders.append(order_id)
                remain_time_orders[order_id] = delta_time
            elif order_id in next_orders:
                current_window_orders.append(order_id)
                
            if len(current_window_orders) >= MAX_ORDER_SINGLE_WINDOW:
                break
        ...
        process_list = import_orders if is_do_import else export_orders
        #print(f"Orders in window: {len(process_list)} Do import: {is_do_import}")
     
        
        # Sort order by due datetime
        #process_list.sort(key=lambda x: orders[x].due_datetime)
        ...
        for order_id in process_list:
            ...
            delta_time = remain_time_orders[order_id]
            
            load_ratio = order.demand / MAX_ORDER_DEMAND
            load_ratio = max(load_ratio, 1)
            
            rate = order.get_total_rate_hours()
            final_rate = (24*EXPECT_DAYS)*(rate*0.75/MEAN_RATE);
            max_capacity = int(MAX_ORDER_DEMAND*delta_time/final_rate)
            max_capacity = min(MAX_ORDER_DEMAND, int(max_capacity * load_ratio))
            is_assign_capacity = before_remain <= max_capacity
            max_capacity = min(max_capacity, before_remain)
            
            
            #print("DEBUG Max Capacity", max_capacity, "Is Assign Capacity", is_assign_capacity, EXPECT_DAYS, delta_time, final_rate)
            ...
            remaining_load_demand_order_ids[order_id] -= total_load
            
            for barge_info in  assigned_barges:
                barge = barge_info['barge']
                barge_info['assigned_order'] = order_id
               
            #print("assign barge", assigned_barges[0])
            
            all_assign_barges.extend(assigned_barges)
            
            if remaining_load_demand_order_ids[order_id] <= 0:
                remaining_load_demand_order_ids[order_id] = 0
                print(f"  ✓ {order_id} [{order.order_type.name}]  {before_remain} {len(barge_ids)} {barge_ids}") if DEBUG_SCHEDULE else None
                remaining_orders.remove(order_id)
                if order_id in next_orders:
                    next_orders.remove(order_id)
            
            elif remaining_load_demand_order_ids[order_id] > 0:
                print(f"  ✓ {order_id} [{order.order_type.name}]  {order.demand -remaining_load_demand_order_ids[order_id]} {len(barge_ids)} {barge_ids}") if DEBUG_SCHEDULE else None
                print(f"  ✗ {order_id} - Nex round",  remaining_load_demand_order_ids[order_id], process_list)if DEBUG_SCHEDULE else None
                next_orders.add(order_id)
                
            else:
                raise Exception("Remaining load demand order ids is not zero")
    ...
    def _display_update_barges(self, assigned_barges, message):
        ...
        for barge_info in  assigned_barges:
            ...
            order = orders[order_id]
            station = order.start_object.station
            barge_scheule_info = self.barge_scheule[barge.barge_id][-1]
    
            print("Barge", barge.barge_id, barge.capacity, order_id, barge_info['load'], barge_scheule_info['station_id'], barge_scheule_info['end_datetime'])
            total_capacity += barge.capacity
            total_load += barge_info['load']
    ...
    def __display_tugboat_results(self, tugboat_results, message, isFull=True):
        ...
        for tugboat_result in tugboat_results:
            ...
            for data_point in tugboat_data_points:
                print("\t",data_point.name, data_point.station_id, data_point.enter_datetime, data_point.exit_datetime, 
                      data_point.type_point, data_point.order_ids)
            #break
        print("######### Total Capacity", total_capacity, "Total Load", total_load)
        print("")
    ...
    def generate_schedule_v2(self, order_ids , xs = None):
        ...
        while len(remaining_orders) > 0:
            ...
            befor_remaining_orders = remaining_load_demand_order_ids.copy()
            
            assigned_barges = self.assign_barges_single_window(remaining_orders, assigned_orders, remaining_load_demand_order_ids, 
                                                start_travel_datetime, target_travel_datetime, next_orders, is_do_import)
         
            current_assign_barge_load = sum(b['load'] for b in assigned_barges)
            ...
            lookup_order_barges = {}
            for assigned_barge in assigned_barges:
                barge = assigned_barge['barge']
                order_id = assigned_barge['assigned_order'] 
                order = orders[order_id]
                station = order.start_object.station
                assigned_barge_order_ids[barge.barge_id] = order_id
                if order_id not in lookup_order_barges:
                    barges = []
                    lookup_order_barges[order_id] = barges
                else:
            ...
            # if  not is_completed_route:
            #     target_travel_datetime = start_travel_datetime + timedelta(days=5)
            #print(f"\n=== STEP {step_count}: {start_travel_datetime} to {target_travel_datetime} {len(assigned_barges) } {total_load} ===")
            ...
            # Safety breaks
            if step_count > 300 or start_travel_datetime > end_date+ timedelta(days=45):
                break
        ...
        self.__display_timeline(order_ids, start_date, end_date, load_order_checks);
        all_dfs = []
        for tugboat_result in all_result_tugboats:
            df = pd.DataFrame([data_point.to_dict() for data_point in tugboat_result['data_points']])
              
            df['water_type']= 'None'
            all_dfs.append(df)
            total_load_tugboat_order += tugboat_result['data_points'][0].total_load
